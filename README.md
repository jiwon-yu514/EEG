...
> ⚠️ 아래는 **README용 예시 구조**입니다.  
> 실제 코드/파일 구조에 맞게 사용자께서 직접 수정해 주세요.

```text
.
├── data/
│   ├── raw/           # BIDS 형식 원본 HBN-EEG (CCD 포함)
│   ├── processed/     # 전처리 완료된 EEG (N, C, T)
│   └── metadata/      # 피험자 정보, 이벤트 주석 등
├── src/
│   ├── preprocessing/ # 필터링, 재표본화, epoch 추출, 정규화 스크립트
│   ├── models/        # EEGNet, 회귀 헤드 구현
│   ├── train.py       # 학습 루프 (Release 조합별 학습)
│   ├── eval.py        # 제로-샷 평가 스크립트
│   └── utils/         # 공통 유틸 함수
├── notebooks/         # 실험 로그 분석, 시각화용 노트북
├── results/
│   ├── logs/          # 학습 및 평가 로그
│   ├── figures/       # 결과 그래프, 예시 플롯
│   └── tables/        # Release 조합별 성능 테이블
└── README.md
